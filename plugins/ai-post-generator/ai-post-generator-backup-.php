...
class AI_Post_Generator {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            'ai-post-generator',
            __('Gerar Imagem', 'ai-post-generator'),
            __('Gerar Imagem', 'ai-post-generator'),
            'upload_files',
            'ai-post-generator-images',
            array($this, 'render_image_generator_page')
        );
    ...
    public function ajax_generate_post() {
        ...
        if ($data['generate_image']) {
            error_log('AIPG: Tentando gerar imagem para post ID: ' . $post_id);
            $image_id = $this->generate_featured_image($data['topic'], $post_id);
            if ($image_id && !is_wp_error($image_id)) {
                set_post_thumbnail($post_id, $image_id);
                error_log('AIPG: Imagem definida como destaque. Image ID: ' . $image_id);
            } else {
                $error = is_wp_error($image_id) ? $image_id->get_error_message() : 'Erro desconhecido';
                error_log('AIPG: Falha ao gerar imagem: ' . $error);
            }
        }
    ...
    /**
     * AJAX handler para gerar imagem (separado)
     */
    public function ajax_generate_image() {
        ...
        // Log de debug
        error_log('AIPG: Iniciando geração de imagem para: ' . $topic);
        
        $image_id = $this->generate_featured_image($topic, $post_id);
        
        if ($image_id && !is_wp_error($image_id)) {
            error_log('AIPG: Imagem gerada com sucesso. ID: ' . $image_id);
            wp_send_json_success(array(
            ...
            $error_msg = is_wp_error($image_id) ? $image_id->get_error_message() : __('Erro ao gerar imagem', 'ai-post-generator');
            error_log('AIPG: Erro ao gerar imagem: ' . $error_msg);
