...
function nosfirnews_render_hero_section( $post_id = null ) {
    ...
    // Ensure array and safe defaults to avoid notices
    $hero_section = is_array( $hero_section ) ? $hero_section : array();
    $enabled = ! empty( $hero_section['enabled'] );
    if ( ! $enabled ) {
        return;
    }

    $background_image = isset( $hero_section['background_image'] ) ? $hero_section['background_image'] : '';
    $overlay_opacity = isset( $hero_section['overlay_opacity'] ) ? floatval( $hero_section['overlay_opacity'] ) : 0.5;
...
function nosfirnews_render_cta_section( $post_id = null ) {
    ...
    // Ensure array and safe defaults to avoid notices
    $cta_section = is_array( $cta_section ) ? $cta_section : array();
    $enabled = ! empty( $cta_section['enabled'] );
    if ( ! $enabled ) {
        return;
    }
    ...
    <div class="nosfirnews-cta-section" style="background-color: <?php echo esc_attr( $background_color ); ?>; color: <?php echo esc_attr( $text_color ); ?>;">
        <div class="container">
            <div class="cta-content">
                ...
                <?php if ( ! empty( $cta_section['description'] ) ) : ?>
                    <p class="cta-description"><?php echo esc_html( $cta_section['description'] ); ?></p>
                <?php endif; ?>
                
                <?php if ( ! empty( $cta_section['button_text'] ) && ! empty( $cta_section['button_url'] ) ) : ?>
