...
function nosfirnews_customize_register( $wp_customize ) {
    ...
    if ( class_exists( 'WooCommerce' ) ) {
        $wp_customize->add_section( 'nosfirnews_woocommerce_options', array(
            'title'    => esc_html__( 'WooCommerce', 'nosfirnews' ),
            'panel'    => 'nosfirnews_theme_options',
            'priority' => 50,
        ) );

        // Banner/Carousel
        $wp_customize->add_setting( 'nosfirnews_woo_banner_enable', array(
            'default'           => false,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_banner_enable', array(
            'label'   => esc_html__( 'Ativar Banner/Carrossel de Produtos', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_banner_source', array(
            'default'           => 'featured',
            'sanitize_callback' => 'nosfirnews_sanitize_select',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_banner_source', array(
            'label'   => esc_html__( 'Fonte do Banner', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'select',
            'choices' => array(
                'featured' => esc_html__( 'Produtos em Destaque', 'nosfirnews' ),
                'sale'     => esc_html__( 'Produtos em Promoção', 'nosfirnews' ),
            ),
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_banner_autoplay', array(
            'default'           => true,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_banner_autoplay', array(
            'label'   => esc_html__( 'Autoplay no carrossel', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_banner_speed', array(
            'default'           => 4000,
            'sanitize_callback' => 'absint',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_banner_speed', array(
            'label'       => esc_html__( 'Velocidade do autoplay (ms)', 'nosfirnews' ),
            'section'     => 'nosfirnews_woocommerce_options',
            'type'        => 'number',
        ...
        $wp_customize->add_setting( 'nosfirnews_woo_search_enable', array(
            'default'           => true,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_search_enable', array(
            'label'   => esc_html__( 'Ativar Busca de Produtos Avançada', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_search_filter_category', array(
            'default'           => true,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_search_filter_category', array(
            'label'   => esc_html__( 'Mostrar filtro de categoria', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_search_filter_price', array(
            'default'           => true,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_search_filter_price', array(
            'label'   => esc_html__( 'Mostrar filtro de preço', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'nosfirnews_woo_search_filter_tag', array(
            'default'           => false,
            'sanitize_callback' => 'nosfirnews_sanitize_checkbox',
        ) );
        $wp_customize->add_control( 'nosfirnews_woo_search_filter_tag', array(
            'label'   => esc_html__( 'Mostrar filtro de tags', 'nosfirnews' ),
            'section' => 'nosfirnews_woocommerce_options',
            'type'    => 'checkbox',
        ) );
    }
