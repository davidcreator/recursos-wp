...
class NosfirNews_Media_Gallery {
    ...
    public function gallery_meta_box_callback($post) {
        ...
        // Garantir que todas as chaves esperadas existam, prevenindo notices de índice indefinido
        ...
        // Mescla os valores salvos com os padrões para evitar chaves ausentes
        if (function_exists('wp_parse_args')) {
            $gallery_settings = wp_parse_args($gallery_settings, $default_settings);
        } else {
            $gallery_settings = $gallery_settings + $default_settings; // fallback simples
        }
        ?>
    ...
    public function ajax_get_gallery_images() {
        ...
        // Mescla com padrões para garantir presença de todas as chaves na resposta
        $default_settings = array(
            'layout' => 'grid',
            'columns' => 3,
            'show_captions' => true,
        ...
        if (function_exists('wp_parse_args')) {
            ...
            $gallery_settings = $gallery_settings + $default_settings;
        }
    ...
    private function render_gallery($gallery_images, $gallery_settings, $extra_class = '') {
        // Padrões consistentes com o admin
        $default_settings = array(
            'layout' => 'grid',
            'columns' => 3,
            'show_captions' => true,
