...
(function($) {
    ...
    function initWooCommerce() {
        ...
        // Initialize homepage banner carousel if present
        initWooBannerCarousel();
    }
...
/**
 * Initialize Woo banner carousel on homepage
 */
function initWooBannerCarousel() {
    var $carouselSection = $('.woo-banner-carousel');
    if (!$carouselSection.length) {
        return;
    }

    var $track = $carouselSection.find('.carousel-track');
    if (!$track.length) {
        return;
    }
    ...
    // Read settings from localized script or data attributes
    var autoplay = (typeof window.nosfirnews_woo_ajax !== 'undefined') ? !!window.nosfirnews_woo_ajax.banner_autoplay : ($carouselSection.data('autoplay') === 'true');
    var speed = (typeof window.nosfirnews_woo_ajax !== 'undefined') ? parseInt(window.nosfirnews_woo_ajax.banner_speed || 4000, 10) : parseInt($carouselSection.data('speed') || 4000, 10);
    ...
    $track.slick({
        ...
        dots: true,
        arrows: true,
        prevArrow: '<button type="button" class="slick-prev" aria-label="Anterior"><i class="fas fa-chevron-left" aria-hidden="true"></i></button>',
        nextArrow: '<button type="button" class="slick-next" aria-label="Próximo"><i class="fas fa-chevron-right" aria-hidden="true"></i></button>',
        adaptiveHeight: true,
        responsive: [
