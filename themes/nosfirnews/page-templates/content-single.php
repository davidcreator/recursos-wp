...
while ( have_posts() ) { the_post();
    ...
    echo '<article id="post-' . get_the_ID() . '" class="single container ' . ( $hover_class !== 'none' ? 'thumb-effect-' . esc_attr( $hover_class ) : '' ) . ' thumb-fit-' . esc_attr( $fit_class ) . '">';
    echo '<header class="single-header">';
    echo '<h1 class="entry-title">' . esc_html( get_the_title() ) . '</h1>';
    $show_single = (bool) get_theme_mod( 'nn_single_thumb_show', true ); $size = (bool) get_theme_mod( 'nn_single_use_cover', true ) ? 'nn_single_cover' : get_theme_mod( 'nn_thumb_size', 'large' );
