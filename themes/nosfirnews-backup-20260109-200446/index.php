 ...
 * @since 1.0.0
 */
$container_class = apply_filters('nosfirnews_container_class_filter', 'container', 'blog-archive');

get_header();

$wrapper_classes = ['posts-wrapper'];
$wrapper_classes = apply_filters('nosfirnews_posts_wrapper_class', $wrapper_classes, 'blog-archive');

$load_before_after_hooks = get_theme_mod('nosfirnews_blog_archive_layout', 'grid') === 'default';
...
<div class="<?php echo esc_attr( $container_class ); ?> archive-container">
		 ...
		 * @since 3.2
		 */
		do_action( 'nosfirnews_do_featured_post', 'index' );
		?>
		...
		<div class="row">
			 ...
			 * @since 1.0.0
			 */
			do_action( 'nosfirnews_do_sidebar', 'blog-archive', 'left' );
			?>
			<div class="nn-index-posts blog col">
				<?php
				/**
				 ...
				 * @since 2.4.0
				 */
				do_action( 'nosfirnews_before_loop' );
				 ...
				 * @since 2.3.10
				 */
				do_action( 'nosfirnews_page_header', 'index' );
				 ...
				 * @since 2.3.10
				 */
				do_action( 'nosfirnews_before_posts_loop' );
				...
				if ( have_posts() ) {
					...
					$pagination_type = get_theme_mod( 'nosfirnews_pagination_type', 'number' );
					...
					nosfirnews_do_loop_hook( 'before' );
					 ...
					 * @since 3.2
					 */
					$excluded_posts = apply_filters( 'nn_exclude_posts', [] );
					...
					while ( have_posts() ) {
						...
						nosfirnews_do_loop_hook( 'entry_before' );
						...
						if ( $load_before_after_hooks ) {
							 ...
							 * @since 2.11
							 */
							do_action( 'nosfirnews_loop_entry_before' );
						}
						...
						if ( $load_before_after_hooks ) {
							 ...
							 * @since 2.11
							 */
							do_action( 'nosfirnews_loop_entry_after' );
						}
						...
						if ( $pagination_type !== 'infinite' && $load_before_after_hooks ) {
							if ( $post_index === $hook_after_post && $hook_after_post !== - 1 ) {
								 ...
								 * @since 2.3.10
								 */
								do_action( 'nosfirnews_middle_posts_loop' );
							}
							$post_index ++;
						}

						nosfirnews_do_loop_hook( 'entry_after' );
					}
					echo '</div>';
					if ( ! is_singular() ) {
						/**
						 ...
						 */
						do_action( 'nosfirnews_do_pagination', 'blog-archive' );
					}
				} else {
				 ...
				 * @since 2.3.10
				 */
				do_action( 'nosfirnews_after_posts_loop' );

				nosfirnews_do_loop_hook( 'after' );
				?>
			</div>
			<?php
			do_action( 'nosfirnews_do_sidebar', 'blog-archive', 'right' );
			?>
		</div>
	</div>
<?php
